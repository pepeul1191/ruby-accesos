class MyApp < Sinatra::Base
  ...
  post '/usuario/guardar_sistemas' do
    ...
    if error == false
      return {:tipo_mensaje => 'success', :mensaje => ['Se ha registrado la asociación/deasociación de los sistemas al usuario', []]}.to_json
    else
      status 500
      return {:tipo_mensaje => 'error', :mensaje => ['Se ha producido un error en asociar/deasociar los sistemas al usuario', execption.message]}.to_json
    end
  end

  post '/usuario/asociar_roles' do
    data = JSON.parse(params[:data])
    nuevos = data['nuevos']
    editados = data['editados']
    eliminados = data['eliminados']
    ...
    DB.transaction do
      begin
        if nuevos.length != 0
          nuevos.each do |nuevo|
            n = UsuarioRol.new(:rol_id => nuevo['id'], :usuario_id => usuario_id)
            n.save
          end
        end
        if eliminados.length != 0
          eliminados.each do |eliminado|
            UsuarioRol.where(:rol_id => eliminado, :usuario_id => usuario_id).delete
          end
        end
      rescue Exception => e
        Sequel::Rollback
    ...
    if error == false
      return {:tipo_mensaje => 'success', :mensaje => ['Se ha registrado la asociación/deasociación de los roles al usuario', []]}.to_json
    else
      status 500
      return {:tipo_mensaje => 'error', :mensaje => ['Se ha producido un error en asociar/deasociar los roles al usuario', execption.message]}.to_json
    end
  end

  post '/usuario/asociar_permisos' do
    data = JSON.parse(params[:data])
	  nuevos = data['nuevos']
 	  editados = data['editados']
	  eliminados = data['eliminados']
	  ...
	  DB.transaction do
			begin
				if nuevos.length != 0
					nuevos.each do |nuevo|
						n = UsuarioPermiso.new(:permiso_id => nuevo['id'], :usuario_id => usuario_id)
						n.save
					end
				end
				if eliminados.length != 0
					eliminados.each do |eliminado|
						UsuarioPermiso.where(:permiso_id => eliminado, :usuario_id => usuario_id).delete
					end
				end
			rescue Exception => e
				error = true
		...
		if error == false
			{:tipo_mensaje => 'success', :mensaje => ['Se ha registrado la asociación/deasociación de los permisos al usuario', []]}.to_json
		else
      status 500
			{:tipo_mensaje => 'error', :mensaje => ['Se ha producido un error en asociar/deasociar los permisos al usuario', execption.message]}.to_json
		end
  end
end
